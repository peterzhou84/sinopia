...
module.exports = function(config, auth, storage) {
  ...
  app.post('/-/changepwd', function(req, res, next) {
    auth.authenticate(req.body.user, req.body.oldpwd, function(err, user) {
      if (!err && user) {
        // success
        if(req.body.newpwd == req.body.newpwd2){
          auth.set_pwd(req.body.user, req.body.newpwd,function(err, user){
              if(!err && user){
                  // success
                  res.send({status:0,msg:"success"});
                  return;
              }
              res.send({status:1,msg:"Change password failed because internal set operation failed."});
          });
        }else{
          res.send({status:2,msg:"The two new passwords are not the same."});
        }
        return;
      }
      res.send({status:3,msg:"Change password failed, please check your current password and try again."});
    })
  })
